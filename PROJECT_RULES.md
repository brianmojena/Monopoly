...
- **Plataforma**: iOS nativo, Swift + SwiftUI. Versión mínima **iOS/iPadOS 18**, para que jueguen dispositivos más viejos; lo que solo exista en versiones nuevas va detrás de `if #available`, así esos dispositivos lo siguen teniendo. Se parte del proyecto Xcode ya presente en el repo (`Monopoly.xcodeproj` / carpeta `Monopoly/`).
...
- **Pagos por proximidad (opcional)**: iOS no permite NFC entre dos iPhone (Core NFC solo lee etiquetas y la emulación de tarjeta requiere un acuerdo comercial con Apple). Como alternativa, Brian aprobó usar **NearbyInteraction (UWB)** para "pagar acercando iPhones": los tokens de descubrimiento se intercambian por la misma sesión MultipeerConnectivity (el host reenvía las señales entre clientes) y la proximidad solo identifica al jugador que cobra (a ≤ 25 cm durante varias lecturas seguidas, antes de que los iPhone se toquen, porque juntarlos por arriba abre NameDrop de iOS y tapa la app; el pago se envía solo al detectarlo); el pago en sí sigue siendo un `GameIntent` validado por el host. Se activa por partida desde la configuración del host y está **desactivado por defecto**; los botones de pago normales siempre siguen disponibles. Requiere iPhone 11 o posterior (excepto SE) en ambos jugadores.
- **Pagos por QR**: siempre disponibles (no requieren activarse ni hardware especial, solo la cámara). Quien cobra muestra un QR y quien paga lo escanea con la cámara (AVFoundation); el pago se envía en cuanto se lee el QR, sin paso de confirmación; el QR se genera con Core Image. El QR solo identifica **a quién o qué propiedad** se paga (y opcionalmente un monto fijo); el pago en sí sigue siendo el mismo `GameIntent` (`collectRent` o `transferMoney`) validado por el host, así que las reglas de turno, saldo y bancarrota no cambian. El formato del QR es `monopoly-pay:1:rent:<propertyID>` o `monopoly-pay:1:transfer:<playerID>:<monto o 0>`; un QR de otra partida no se reconoce porque sus IDs no existen en esta.
...
- **Persistencia**: el host guarda cada partida que aloja (`GameState` + qué jugadores juegan desde su iPhone) como JSON `Codable` en Application Support (`saved-games/<roomID>.json`, `GameStore`) después de cada cambio de estado. Los clientes no guardan el estado (el host es la fuente de verdad), solo lo necesario para volver como el mismo jugador (`JoinedGamesStore`: sala, ID de su jugador, nombre del host, ronda). Inicio lista ambas en **Partidas recientes** ("Continuar partida" / "Volver a entrar").
- **Navegación de una partida**: la partida activa vive en `AppModel`, no en una pantalla apilada: `ContentView` muestra `ActiveGameView` en lugar de Inicio, sin botón atrás ni gesto para salir. Solo se sale con "Salir" (con confirmación; el host elige entre guardar o borrar). La app recuerda la partida activa y, si se cierra, vuelve directo a ella al reabrirse. Al volver a primer plano se reinicia el anuncio/búsqueda de red, y un cliente sin conexión reintenta solo hasta reconectar.
- **Ajustes globales** (`AppSettings`, pantalla Ajustes): el nombre del jugador (se usa al alojar y al unirse, sin escribirlo en cada partida) y mantener la pantalla encendida durante la partida (activado por defecto, porque bloquear el iPhone lo desconecta).
